...
AUTH_BOOTSTRAP_PASSWORD=<set-in-render>
HR_ADMIN_TEST_PASSWORD=<set-in-render>
EMPLOYEE_TEST_PASSWORD=<set-in-render>
...
|---|---|---|
| HR admin | as seeded in your `employees` collection | configured in environment || Employee | their Employee ID | their own Employee ID (e.g. `EMP000001`) |
...
1. **Rotate the Atlas database password before production deployment.** 
     Never commit database credentials or production secrets to the repository. Store production values only in the hosting platform's environment variables.
